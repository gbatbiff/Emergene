...
#tr <- read.nexus("test/phylo/tree.nex")
tr <- read.nexus(tree_file)

# Tidy up the names of the samples at the tips
tr$tip.label <- gsub(pattern = "'", replacement = "", x = tr$tip.label)
tr$tip.label <- str_extract(string = tr$tip.label,
                            pattern = regex("(SRR[^_]*|.*_.*#[^_]*)"))

tr$tip.label <- stringr::str_remove(string = tr$tip.label, pattern = "#")
tr$tip.label <- stringr::str_remove(string = tr$tip.label, pattern = "_")
tr$tip.label <- paste0("A", tr$tip.label)

# Read in AMR GME summary data

#dt <- fread("test/metadata/amr.csv")
dt<-fread(amr_table_file)

# Rename columns
sdt <- dt[, .(lane = Lane, species = Species,
              accno = `Accession number`,
              azithro = Azithro,
              cipro = Cipro, esbl = ESBL)]

# Filter data in the AMR summary to just the samples at the tree tips
sdt$lane <- str_remove(string = sdt$lane, pattern = "#")
sdt$lane <- str_remove(string = sdt$lane, pattern = "_")
sdt$lane <- paste0("A", sdt$lane)
sdt <- sdt[lane %in% tr$tip.label,]


# Drop tips from tree that are not in the data
tr <- drop.tip(tr, tr$tip.label[!(tr$tip.label %in% sdt$lane)])

# Make AMR columns factors
cols <- colnames(sdt)[4:6]
sdt <- sdt[, (cols) := lapply(.SD, FUN = function(x){factor(x, levels = c( "No", "Yes"))}), .SDcols = cols]


sdtm <- as.matrix(sdt[, 4:6])
rownames(sdtm) <- sdt$lane
colnames(sdtm) <- cols
...
for (amr in cols) {
  ...
  # create temporary data without missing entries
  tdt <- sdt[, .(lane, mge = get(amr))]
  # Drop missing entries from tree
  drop_tips <- tdt[is.na(mge), lane]
  sub_tr <- drop.tip(tr, tip = drop_tips)
  tdt <- tdt[!is.na(mge), ]
  tdt<-unique(tdt)
  tree<-sub_tr ### IF I WANT TO LAUNCH THE ANC STATE ON ENTIRE TREE
  
  #tree<-multi2di(tree) # remove multifurcations
  tdt_1<-tdt
  tdt_1<-tdt_1[which(tdt_1$lane %in% tree$tip.label),]
  # Drop missing entries from tree
  drop_tips <- tdt_1[is.na(mge), lane]
  tdt_1 <- tdt_1[!is.na(mge), ]
  ...
  # aa<-extract.clade(tree, 273)
  # xx[names(xx) %in% aa$tip.label]<-"R"
